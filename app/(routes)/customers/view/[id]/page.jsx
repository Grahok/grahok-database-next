...
import { fetchEntries } from "@/app/(routes)/entries/all/actions";
            ...
            <tbody className=" [&_tr:nth-child(even)]:bg-gray-100">
              {entries.map((entry, index) => (
                <tr key={entry._id}>
                  ...
                  <td>{new Date(entry.orderDate).toLocaleDateString()}</td>
                  <td>{entry.paidByCustomer}</td>
                  <td>{entry.status || "Unknown"}</td>
                  <td>
                    <div className="flex gap-1">
                      <a className="p-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition duration-200">
                        <FaEye />
                      </a>
                      <a className="p-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition duration-200">
                        <FaPencil />
                      </a>
